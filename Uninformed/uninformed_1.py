...
class Box(Problem):
    ...
    def goal_test(self, state):
        man_pos,box_1,box_2 = state
        return box_1 == (4,4) and box_2 == (4,4)

    @staticmethod
    def check_valid(state):
        man_pos,box_1,box_2 = state
        man_x,man_y = man_pos
        box_1_x,box_1_y = box_1
        box_2_x,box_2_y = box_2

        if man_x < 0 or man_x >= 5 or man_y < 0 or man_y >= 5:
            return False
        if box_1 == box_2 and box_1 != (4,4):
            return False
        if box_1_x < 0 or box_1_x >= 5 or box_1_y < 0 or box_1_y >= 5:
            return False
        if box_2_x < 0 or box_2_x >= 5 or box_2_y < 0 or box_2_y >= 5:
            return False
    ...
    def successor(self, state):
        ...
        for move,(x,y) in movement.items():
            ...
            new_box_1 = (box_1[0]+x,box_1[1]+y)
            new_box_2 = (box_2[0]+x,box_2[1]+y)
            if new_man_pos == box_1:
                if self.check_valid((new_man_pos,new_box_1,box_2)):
                    succ[f"Push box 1 {move}"] = (new_man_pos,new_box_1,box_2)
            elif new_man_pos == box_2:
                if self.check_valid((new_man_pos,box_1,new_box_2)):
                    succ[f"Push box 2 {move}"] = (new_man_pos,box_1,new_box_2)
            else:
                if self.check_valid((new_man_pos,box_1,box_2)):
                    succ[f"Move man {move}"] = (new_man_pos,box_1,box_2)
