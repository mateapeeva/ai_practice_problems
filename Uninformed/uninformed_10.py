            ...
            for col in range(self.n):
                ...
                for (x,y) in neigh:
                    ...
                    if self.check_valid((nx,ny)):
                        index = nx * self.n + ny
                        new_matrix[index] = 1 - new_matrix[index]

                succ[f'x: {row}, y: {col}'] = tuple(new_matrix)
...
if __name__ == "__main__":
    n = int(input())
    fields_arr = list(map(int, input().split(',')))

    initial = tuple(fields_arr)
    problem = Board(initial,n)
