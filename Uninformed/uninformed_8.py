...
class Disks(Problem):
    def __init__(self, initial,l,n, goal=None):
    ...
    def successor(self, state):
        succ = {}
        disks_pos = list(state)
        movements = {"R1": 1, "R2": 2, "L1": -1, "L2": -2}
        for i in range(len(disks_pos)):
            if disks_pos[i] == 0:
                continue
            for move,add in movements.items():
                new_disks_pos = [k for k in disks_pos]
                new_pos = i
                if move == "R1" and new_pos + add < self.l and new_disks_pos[new_pos + add] == 0:
                    new_pos += add
                    new_disks_pos[new_pos] = new_disks_pos[i]
                    new_disks_pos[i] = 0
                    disks = tuple(new_disks_pos)
                    succ[f"{move}: Disk {new_disks_pos[new_pos]}"] = (disks)
                elif move == "R2" and new_pos + add < self.l and new_disks_pos[new_pos + add] == 0 and new_disks_pos[new_pos + add - 1] != 0:
                    new_pos += add
                    new_disks_pos[new_pos] = new_disks_pos[i]
                    new_disks_pos[i] = 0
                    disks = tuple(new_disks_pos)
                    succ[f"{move}: Disk {new_disks_pos[new_pos]}"] = (disks)
                elif move == "L1" and new_pos + add >= 0 and new_disks_pos[new_pos + add] == 0:
                    new_pos += add
                    new_disks_pos[new_pos] = new_disks_pos[i]
                    new_disks_pos[i] = 0
                    disks = tuple(new_disks_pos)
                    succ[f"{move}: Disk {new_disks_pos[new_pos]}"] = (disks)
                elif move == "L2" and new_pos + add >= 0 and new_disks_pos[new_pos + add] == 0 and new_disks_pos[new_pos + add + 1] != 0:
                    new_pos += add
                    new_disks_pos[new_pos] = new_disks_pos[i]
                    new_disks_pos[i] = 0
                    disks = tuple(new_disks_pos)
