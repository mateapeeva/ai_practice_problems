...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for move, (x, y) in movement.items():
            one_new = (one[0] + x, one[1] + y)
            if self.check_valid((one_new, two, three, four, five)):
                succ[f"Move square 1 {move}"] = (one_new, two, three, four, five)

        for move, (x, y) in movement.items():
            two_new = (two[0] + x, two[1] + y)
            if self.check_valid((one, two_new, three, four, five)):
                succ[f"Move square 2 {move}"] = (one, two_new, three, four, five)

        for move, (x, y) in movement.items():
            three_new = (three[0] + x, three[1] + y)
            if self.check_valid((one, two, three_new, four, five)):
                succ[f"Move square 3 {move}"] = (one, two, three_new, four, five)

        for move, (x, y) in movement.items():
            four_new = (four[0] + x, four[1] + y)
            if self.check_valid((one, two, three, four_new, five)):
                succ[f"Move square 4 {move}"] = (one, two, three, four_new, five)

        for move, (x, y) in movement.items():
            five_new = (five[0] + x, five[1] + y)
            if self.check_valid((one, two, three, four, five_new)):
                succ[f"Move square 5 {move}"] = (one, two, three, four, five_new)
