...
class Balls(Problem):
    def __init__(self, initial, n, invalid_pos):
        super().__init__(initial)
        self.n = n
        self.invalid_pos = set(invalid_pos)

    def check_valid(self, pos):
        x, y = pos
        return 0 <= x < self.n and 0 <= y < self.n and pos not in self.invalid_pos

    def successor(self, state):
        succ = {}
        balls_pos, num_balls = state
        movement = {
        ...
        current_balls = set(balls_pos)

        for ball in balls_pos:
            for move_name, (dx, dy) in movement.items():
                over_pos = (ball[0] + dx, ball[1] + dy)
                land_pos = (ball[0] + 2 * dx, ball[1] + 2 * dy)

                if over_pos in current_balls and self.check_valid(land_pos) and land_pos not in current_balls:
                    new_balls_set = set(current_balls)
                    new_balls_set.remove(ball)
                    new_balls_set.remove(over_pos)
                    new_balls_set.add(land_pos)

                    new_state = (tuple(sorted(list(new_balls_set))), num_balls - 1)
                    succ[f"{move_name}: (x={ball[0]},y={ball[1]})"] = new_state
    ...
    def goal_test(self, state):
        balls_pos, num_balls = state
        goal_pos = (self.n // 2, self.n - 1)
        return num_balls == 1 and balls_pos == (goal_pos,)
...
if __name__ == "__main__":
    ...
    for _ in range(num_balls):
        balls_input.append(tuple(map(int, input().split(','))))

    num_invalid = int(input())
    invalid_input = []
    for _ in range(num_invalid):
        invalid_input.append(tuple(map(int, input().split(','))))

    initial_state = (tuple(sorted(balls_input)), num_balls)

    problem = Balls(initial_state, n, invalid_input)
