...
class Pacman(Problem):
    ...
    def check_valid(self, state):
        ...
        if man in self.obstacles:
            return False
        if not 0 <= man[0] <= 9 or not 0 <= man[1] <= 9:
            return False
    ...
    def successor(self, state):
        succ = {}
        man,move,eating = state
        x,y = move
        movement = {"Move forward":(x,y),"Move backward":(-x,-y),"Turn left":(-y,x),"Turn right":(y,-x)}

        for action,(nx,ny) in movement.items():
            new_eating = list(eating)
            move_new = (nx,ny)
            new_man = (man[0]+nx,man[1]+ny)

            if self.check_valid(new_man):
                if new_man in eating:
                    new_eating.remove(new_man)
                succ[action] = (new_man, move_new, tuple(new_eating))
...
if __name__ == "__main__":
    ...
    num_eat = int(input())
    eating = []
    for _ in range(num_eat):
        eating.append(tuple(map(int,input().split(','))))

    initial = (man,move,tuple(eating))

    problem = Pacman(initial, obstacles, goal=man)
