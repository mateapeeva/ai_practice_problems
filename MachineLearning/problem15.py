...
if __name__ == '__main__':
    ...
    split_index_0 = int(len(dataset_0)*x/100.0)
    split_index_1 = int(len(dataset_1)*x/100.0)
    ...
    acc_nb = classifier_nb.score(test_X,test_y)

    params = {
        'criterion':'entropy',
        'random_state':0,
    }
    classifier_dt = DecisionTreeClassifier(**params)
    classifier_dt.fit(train_X,train_y)
    acc_dt = classifier_dt.score(test_X,test_y)
    ...
    acc_forest = classifier_forest.score(test_X,test_y)
    ...
    classifier_nn = MLPClassifier(**params)
    classifier_nn.fit(train_X,train_y)
    acc_nn = classifier_nn.score(test_X,test_y)

    maxi = max(acc_nn,acc_dt,acc_forest,acc_nb)
    classifier = ""

    w_nb, w_dt, w_forest, w_nn = 1,1,1,1
    ...
    print(f"Najgolema tocnost ima klasifikatorot {classifier}")

    pred_nb = classifier_nb.predict(test_X)
    pred_nn = classifier_nn.predict(test_X)
    pred_forest = classifier_forest.predict(test_X)
    pred_dt = classifier_dt.predict(test_X)

    votes_for_one = (pred_nb*w_nb) + (pred_dt*w_dt) + (pred_forest*w_forest) + (pred_nn*w_nn)

    final_pred = []
    for vote in votes_for_one:
        if vote >= 3:
            final_pred.append(1)
        else:
            final_pred.append(0)

    recall = recall_score(test_y,final_pred)
