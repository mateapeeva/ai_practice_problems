...
if __name__ == '__main__':
    ...
    split_index = int(len(data)*x/100.0)
    ...
    least_imp_attribute = classifier.feature_importances_.argmin()

    #Without col
    data = [[el for i,el in enumerate(row) if i != least_imp_attribute] for row in data]
    ...
    train_X_ss = scaler_SS.fit_transform(train_X)
    test_X_ss = scaler_SS.transform(test_X)

    train_X_mms = scaler_MMS.fit_transform(train_X)
    test_X_mms = scaler_MMS.transform(test_X)
    ...
    classifier_NN.fit(train_X_ss,train_y)
    acc_ss = classifier_NN.score(test_X_ss,test_y)

    classifier_NN.fit(train_X_mms,train_y)
    acc_mms = classifier_NN.score(test_X_mms,test_y)

    print(f"Tocnost so StandardScaler: {acc_ss}")
    print(f"Tocnost so MinMaxScaler: {acc_mms}")
