...
# Forrest with 85% train, 15% test, remove col-index
# Input: col-index, number of trees, criterion, sample
# random_state = 0
# Output: accuracy, predicted class, probabilities
...
sample = [el for i,el in enumerate(sample) if i != col_index]

X,y = [[el for i,el in enumerate(row[:-1]) if i != col_index] for row in dataset], [row[-1] for row in dataset]
...
prediction_sample = classifier.predict([sample])[0]
probabilities_sample = classifier.predict_proba([sample])[0]
