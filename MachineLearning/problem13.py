...
if __name__ == '__main__':
    ...
    dataset_2 = [row for row in dataset if row[-1] == 2]
    ...
    X_2,y_2 = [row[:-1] for row in dataset_2],[row[-1] for row in dataset_2]

    # SPLIT FOR NB
    split_index_0 = int(len(dataset_0)*x1)
    split_index_1 = int(len(dataset_1)*x1)
    split_index_2 = int(len(dataset_2)*x1)

    train_X_nb = X_0[:split_index_0] + X_1[:split_index_1] + X_2[:split_index_2]
    train_y_nb = y_0[:split_index_0] + y_1[:split_index_1] + y_2[:split_index_2]

    classifier_nb = GaussianNB()
    classifier_nb.fit(train_X_nb,train_y_nb)

    # SPLIT FOR DT
    split_index_0_x2 = int(len(dataset_0) * x2)
    split_index_1_x2 = int(len(dataset_1) * x2)
    split_index_2_x2 = int(len(dataset_2) * x2)

    train_X_dt = X_0[split_index_0:split_index_0_x2] + X_1[split_index_1:split_index_1_x2] + X_2[split_index_2:split_index_2_x2]
    train_y_dt = y_0[split_index_0:split_index_0_x2] + y_1[split_index_1:split_index_1_x2] + y_2[split_index_2:split_index_2_x2]

    classifier_dt = DecisionTreeClassifier(random_state=0)
    classifier_dt.fit(train_X_dt,train_y_dt)

    # SPLIT FOR FOREST
    train_X_forest = X_0[:split_index_0_x2] + X_1[:split_index_1_x2] + X_2[:split_index_2_x2]
    train_y_forest = y_0[:split_index_0_x2] + y_1[:split_index_1_x2] + y_2[:split_index_2_x2]

    classfier_forest = RandomForestClassifier(random_state=0,n_estimators=3)
    classfier_forest.fit(train_X_forest,train_y_forest)

    test_X = X_0[split_index_0_x2:] + X_1[split_index_1_x2:] + X_2[split_index_2_x2:]
    test_y = y_0[split_index_0_x2:] + y_1[split_index_1_x2:] + y_2[split_index_2_x2:]

    pred_nb = classifier_nb.predict(test_X)
    pred_dt = classifier_dt.predict(test_X)
    pred_forest = classfier_forest.predict(test_X)

    true_y = test_y

    tr_pr = 0
    for pr_nb,pr_dt,pr_forest,tr_y in zip(pred_nb,pred_dt,pred_forest,true_y):
        count = 0
        if pr_nb == tr_y:
            count += 1
        if pr_dt == tr_y:
