...
class Snake(Problem):
    ...
    def check_valid(self,state):
        ...
        if snake_head in snake_body[1:]:
            return False
        if not 0 <= snake_head[0] < 10 or not 0 <= snake_head[1] < 10:
            return False
    ...
    def successor(self, state):
        ...
        snake_body,green_apples,current_dir = state
        head = snake_body[0]
        ...
        for action in actions:
            if action == "Move forward":
                ...
                new_dir = dirs[(dirs.index(current_dir)+1)%4]
            elif action == "Turn left":
                new_dir = dirs[(dirs.index(current_dir)-1)%4]

            new_head = (head[0]+move[new_dir][0], head[1]+move[new_dir][1])
            new_snake_body = [new_head]+snake_body
            new_green_apples = [apple for apple in green_apples]

            if new_head in new_green_apples:
                new_green_apples.remove(new_head)
            else:
                new_snake_body.pop(-1)

            if self.check_valid(new_snake_body):
                succ[action] = (tuple(new_snake_body), tuple(new_green_apples), new_dir)

        return succ

    def h(self, node):
        snake_body, green_apples,dir = node.state
        snake_head = snake_body[0]
        mini = 0
        for apple in green_apples:
            value = abs(snake_head[0] - apple[0]) + abs(snake_head[1] - apple[1])
            mini = min(value, mini)

        return mini + len(green_apples)
...
if __name__ == '__main__':
    ...
    dir = "Down"
    initial = (snake_body, tuple(green_apples), dir)
