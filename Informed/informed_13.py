...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        ...
        if ghost_pos in walls:
            return False
        if not 0 <= ghost_pos[0] < n or not 0 <= ghost_pos[1] < n:
            return False
    ...
    def successor(self, state):
        ...
        for move,(x,y) in movement.items():
            for i in range(1,4):
                nx,ny = x*i,y*i
                new_ghost_pos = (ghost_pos[0] + nx, ghost_pos[1] + ny)
                if self.check_valid(new_ghost_pos,self.walls,self.n):
                    successors[f"{move} {i}"] = (new_ghost_pos)
    ...
    def h(self, node):
        ...
        return (abs(pacman_x - ghost_x) + abs(pacman_y - ghost_y)) / 3
