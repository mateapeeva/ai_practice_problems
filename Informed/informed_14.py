...
class Squares(Problem):
    ...
    def h(self, node):
        s1,s2,s3,s4,s5 = node.state

        def manhattan_distance(x,i):
            return abs(x[0]-i[0]) + abs(x[1]-i[1])

        value = manhattan_distance(s1,(0,4)) + manhattan_distance(s2,(1,3)) + manhattan_distance(s3,(2,2)) + manhattan_distance(s4,(3,1)) + manhattan_distance(s5,(4,0))
        return value
